$(document).ready(function () {
  ...
  function createElementPost(post, image) {
                        ...
                        <div class="d-flex align-items-center">
                            ...
                            <div
                                ...
                                <div class="">
                                    ...
                                    <p class="mb-0">${calculatorTimeDifferent(
                                      post.UpdatedAt
                                    )}</p>
                                </div>
    ...
    elementPost += `<div class="d-flex align-items-center justify-content-between">
                            <div class="d-flex align-items-center">
                                <div class="d-flex align-items-center">
                                    <div class="like-post">
                                        <div class="dropdown">
                        ...
                        <div class="d-flex flex-wrap">
                            ...
                            <div class="ms-3">
                                ...
                                <div class="d-flex flex-wrap align-items-center comment-activity">
                                    <span class="time-comment-post"> ${calculatorTimeDifferent(
                                      response.commentator.updatedAt
                                    )} </span>
                                    <a href="#">Thích</a>
                                    <a href="#">Phản hồi</a>
                                </div>
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.isSuccess) {
          // close modal
          var dialog = $(
            `#modal-delete-comment-${formData.get("CommentDetailId")}`
          );
          dialog.modal("hide");
  ...
  $(document).on("click", ".update-comment", function (event) {
    ...
    var comments = $(this).closest('.post-comments');
    var userComment = $(this).closest(".user-comment");
    var contentPost = userComment.find(".content-comment");
    var commentDetailId = $(this).attr("data-value");

    comments.find('.frm-update-comment').each(function() {
        $(this).closest('.item-comment').find('.user-comment').css('display', 'block');
        $(this).remove();
    });
    

    var inputComment = `
        <div class="frm-update-comment">
            <div class="d-flex align-items-center comment-text position-relative">
                <input type="text" class="form-control rounded input-comment input-update-comment" placeholder="Viết bình luận..." value="${contentPost.text()}">
                <div class="comment-attagement d-flex">
                    <div class="comment-attagement-more">
                        <a href="#"><i class="ri-link"></i></a>
                        <a href="#"><i class="ri-user-smile-line"></i></a>
                        <a href="#"><i class="ri-camera-line"></i></a>
                    </div>
                    <button class="btn btn-update-comment" style="padding: 0 10px;" type="button" value="${commentDetailId}">
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-send fa-bold" viewBox="0 0 16 16">
                            <path d="M15.854.146a.5.5 0 0 1 .11.54l-5.819 14.547a.75.75 0 0 1-1.329.124l-3.178-4.995L.643 7.184a.75.75 0 0 1 .124-1.33L15.314.037a.5.5 0 0 1 .54.11ZM6.636 10.07l2.761 4.338L14.13 2.576zm6.787-8.201L1.591 6.602l4.339 2.76 7.494-7.493Z"/>
                        </svg>
                    </button>
            ...
            <a href="#" class="btn-cancel-updatecomment">Hủy</a>
        </div>
        `;
    
    userComment.after(inputComment);
    userComment.css('display', 'none');
  });

  $(document).on('click', '.btn-cancel-updatecomment', function(event) {
    var userComment = $(this).closest('.item-comment').find('.user-comment');
    var inputComment = $(this).closest('.item-comment').find('.frm-update-comment');
  ...
  $(document).on('click', '.btn-update-comment', function() {
    var inputComment = $(this).closest('.item-comment').find('.frm-update-comment');
    var inputChange = inputComment.find('.input-update-comment');

    if(inputChange.val() == "") {
        inputChange.focus();
        if(!inputChange.hasClass('input-danger')) {
            inputChange.attr('title', 'incorrect value');
            inputChange.addClass('input-danger');
        }
        return;
    }
    var formData = new FormData();
    formData.append("ContentComment", inputChange.val());
    formData.append("CommentDetailId", $(this).val());
    updateComment(formData, $(this));
  });

  function updateComment(formData, element) {
    var inputComment = $(element).closest('.item-comment').find('.frm-update-comment');
    var userComment = $(element).closest('.item-comment').find('.user-comment');
    $.ajax({
        url: '/UserPosts/UpdateComment',
        type: 'POST',
        data: formData,
        contentType: false,
        processData: false,
        success: function(response) {
          if(response.isSuccess) {
            userComment.find('.content-comment').text(response.contentComment);
            userComment.css('display', 'block');
            inputComment.remove();
          } else {
            console.log("Update commentdetail", response.errorMsg);
          }
        },
        error: function(err) {
            console.log("Comment detail: ", err);
        }
    });
  }
});
